...
<body class="antialiased bg-[#FEFEFE] min-w-screen min-h-screen overflow-x-hidden text-[#35321B] flex flex-col justify-between">
	<main>
		...
		<section class="max-w-[90rem] mx-auto w-full">
			<div class="p-8">
				<button data-modal-toggle="form-modal" class="block text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="button">
					Add Quote
				</button>
									...
									<tr>
										...
										<td class="border-b border-slate-100 p-4 pr-8 text-slate-500 w-[1%]">
											<div class="flex gap-2">
												<button data-modal-toggle="form-modal" data-type="edit" data-id="<?= $quote->id ?>" data-quote='<?= json_encode($quote->quote) ?>' class="block text-gray-800 bg-yellow-400 hover:bg-yellow-500 font-medium rounded-lg text-sm px-2 py-1.5 text-center" type="button">
													Edit
												</button>
												<button data-delete-id="<?= $quote->id ?>" data-href="<?= base_url('screen/quotes/delete') ?>" class="block text-white bg-red-600 hover:bg-red-700 font-medium rounded-lg text-sm px-2 py-1.5 text-center" type="button">
													Hapus
												</button>
											</div>
										</td>
		...
		<div id="form-modal" data-title="Quote" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
			<div class="relative w-full max-w-xl max-h-full">
				<div class="relative bg-white rounded-lg shadow">
					<button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center" data-modal-hide="form-modal">
