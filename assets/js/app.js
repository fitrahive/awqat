function init() {
  ...
  $(document).on("click", "[data-modal-toggle]", function (evt) {
    ...
    $("#".concat(modal))
      .removeClass("hidden")
      .addClass("justify-center items-center flex");
    ...
    if (type === "edit") {
      ...
      $("#".concat(modal))
        .find("form")
        .attr("action", window.location.href.concat("/update/".concat(id)));
      ...
      for (var item in data) {
        $("#".concat(modal))
          .find('input[name="'.concat(item).concat('"]'))
          .val(JSON.parse(data[item]));
      }
    }
  });
  ...
  $(document).on("click", "[data-modal-hide]", function (evt) {
    ...
    $("#".concat(modal))
      .addClass("hidden")
      .removeClass("justify-center items-center flex");
    $("[modal-backdrop]").remove();
  });
