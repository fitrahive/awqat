...
<body class="screen" style="background-image: url(<?= base_url('assets/img/sheikh-zayed.jpg') ?>)">
	...
	<main class="text-white">
		<section id="current" class="fixed top-[23vh] mx-auto inset-x-0 flex justify-center">
			<div class="py-5 px-6 bg-[#892F3B]/[.8] rounded-lg backdrop-blur-sm" id="next">
				<span class="text-3xl font-medium"><?= $label['fajr'] ?> 00:00</span>
			</div>
		</section>

		<section id="prayer-times" class="px-8 w-screen grid grid-cols-6 gap-4 fixed bottom-[10vh]">
			<?php foreach (shalat() as $key) : ?>
				<div class="p-4 bg-[#105E6B]/[.85] rounded-lg flex flex-col items-center">
					<small class="text-2xl font-extrabold"><?= $label[$key] ?></small>
					<span class="text-4xl font-medium" id="<?= $key ?>">00:00</span>
				</div>
			<?php endforeach ?>
		</section>
	</main>
